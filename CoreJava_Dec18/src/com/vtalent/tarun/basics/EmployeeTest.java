...
public class EmployeeTest {
	...
	public static boolean searchAnEmployee() {
		...
		int x;
		if (employeeArray == null) {
			System.out.println("Array is empty, Please run again and pass the value");
			flag = false;
		} else {
	...
	public static boolean updateAnEmployee() {
		...
		if (employeeArray == null) {
			...
			updEmp = input.nextInt();
			for (int i = 0; i < employeeArray.length; i++) {
				Employee emp = employeeArray[i];
				if (emp.employeeId == updEmp) {
				}
				System.out.println("Do you want to update your details (1/0), 1-Yes, 0-No ");
				m = input.nextInt();
				if (m==1){
					System.out.println("Do you want to update your Salary (1/0), 1-Yes, 0-No");
					m= input.nextInt();
					if (m==1) {
						System.out.println("Enter Salary: ");
						emp.employeeSalary = input.nextFloat();
					...
					//}
					if (m==0) {
					System.out.println("Do you want to update your Mobile No. (1/0), 1-Yes, 0-No ");
					m= input.nextInt();
					if (m==1) {
						System.out.println("Enter Mobile No.: ");
						emp.employeeMobileNo = input.nextLong();
						System.out.println(emp.employeeMobileNo);
						System.out.println("Mobile No. Updated");
					
					}else {
						System.out.println("Check the details and try again");

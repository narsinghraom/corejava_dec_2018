...
public class EmpTest {
	...
	public static boolean insertAnEmployee() {
		...
		if (employeeArray == null) {
			...
			for (int i = 0; i <= employeeArray.length - 1; i++) {
				if (employeeArray[i] != null) {
					EmpInfo employee = employeeArray[i];
	...
	public static void main(String[] args) {
		...
		employeeArray = new EmpInfo[employeeSize];
		while(true) {
			...
			case 4: printAllEmployees();
					break;
			case 5: boolean updateFlag = updatingEmployee();
			if(updateFlag) {
				System.out.println("search successfully");
			}
			else
			{
				System.out.println("invalied id please try again");
			}
			break;

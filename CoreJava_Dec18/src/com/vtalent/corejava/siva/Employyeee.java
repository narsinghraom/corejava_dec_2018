...
class EmployeeOperationn{
	static EmployeeInfoo[] einfoArray;
	Scanner sc = new Scanner(System.in);

	public void insertEmployee() {
		System.out.println("enter the number of employee stored");
		int size = sc.nextInt();
		EmployeeOperationn.einfoArray = new EmployeeInfoo[size];

		for (int i = 0; i <= size - 1; i++) {
			EmployeeInfoo empinfo = new EmployeeInfoo();
	...
	public void selectAllData() {
		if (einfoArray != null) {
			for (int i = 0; i <= einfoArray.length - 1; i++) {
				EmployeeInfoo emp1 = einfoArray[i];
				if (einfoArray[i] == null) {
...
public class Employyeee  {
	static int z=0;
	
	public static void main(String[] args) {
		
		// Employee emp = new Employee();
		EmployeeOperationn empo = new EmployeeOperationn();
		while (true) {

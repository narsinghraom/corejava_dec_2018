...
public class EmployeeOperations {
	...
	public static void upDateEmployeeData() {
		...
		if (empArray != null) {
			...
			int x = EmployeeTest.input.nextInt();
			for (int i = 0; i <= empArray.length - 1; i++) {
				Employee emp = empArray[i];
				if (emp != null) {
					if (x == emp.getEmployeeID()) {
						System.out.println("enter Salary:");
						int x1 = EmployeeTest.input.nextInt();
			...
			System.out.println("plz 1st Enter Data");
		}
	}
		...
		if (empArray != null) {
			...
			for (int i = 0; i <= empArray.length - 1; i++) {
				...
				if (emp != null) {
					if (x == emp.getEmployeeID()) {
						empArray[i] = null;
						System.out.println("Data were Deleted");
						EmployeeTest.z = EmployeeOperations.empArray.length;
						break;
					} else {
						Count = 2;
					}
